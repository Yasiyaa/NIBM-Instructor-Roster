...
## 7. Upcoming Feature Implementations & Expansion Roadmap

When continuing feature development on this codebase, the approved priority roadmap is:

1. **WhatsApp Cadre Dispatcher & Group Broadcast Hub**:
   - Location: Inside `SundayPlanner.tsx`.
...
2. **Academic Calendar Sync (`.ics` Feed)**:
   ...
   - Functionality: Dynamic iCalendar feed allowing instructors to click "Subscribe to Calendar" in `InstructorPortal.tsx` to automatically sync duties to Google Calendar, Apple Calendar, and Outlook.
3. **Executive Audit Trail & Compliance Drawer**:
   - Location: Accessible in `ExecutiveDashboard.tsx` and `Header.tsx`.
   - Functionality: Searchable UI leveraging the existing `AuditLog` table, showing timestamped records of who scheduled duties, approved/rejected leaves, or altered rosters.
...
5. **Dynamic Academic Catalog (Batch & Venue Manager)**:
   - Location: Demonstrator settings modal.
